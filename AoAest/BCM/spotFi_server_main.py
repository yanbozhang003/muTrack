...
eng.addpath(r'/home/project6/work/muTrack/AoAest/BCM/matlab/spotFi_ori/')

### Radio configure
# Set up your radio configure here
# BW = 20/40/80, depending on the channel bandwidth in use
# num_rxAnt = 1/2/3/4, depending on the number of enabled receiving RF chains in usez
...
if BW==20:
    const_pha_diff = sio.loadmat('matlab/data/calib/spotFi_fastcalib_20M_2G_AP2_tr3.mat')
else:
    raise Exception("No phase offset is loaded when BW is larger than 20MHz!")
...
def csiSanitThread():
    ...
    while True:
        if len(DICT_SUM['CSI']) > 0:
            ...
            # CSI_sanit = CSI_matrix_amp * np.exp(1j*pha_sanit)

            CSI_sanit = CSI_matrix_r
...
if __name__ == "__main__":
    ...
    while True:
        if len(DICT_SANIT['CSI']) > 0:
            CSI_complex = DICT_SANIT['CSI'].pop(0)
            CSI_matrix_amp = np.absolute(CSI_complex)
            CSI_matrix_pha = np.angle(CSI_complex)
            
            CSI_matrix_amp_pass = matlab.double(CSI_matrix_amp.tolist())
            CSI_matrix_pha_pass = matlab.double(CSI_matrix_pha.tolist())
            pha_diff_rx12_pass = matlab.double(const_pha_diff['pha_diff_rx12'].tolist())
            pha_diff_rx13_pass = matlab.double(const_pha_diff['pha_diff_rx13'].tolist())

            aoaEstMat = eng.spotFi_func(CSI_matrix_amp_pass,CSI_matrix_pha_pass,pha_diff_rx12_pass,pha_diff_rx13_pass)
    ...
    #             np.save('CSI_data.npy', DICT_SANIT)
    #             os._exit(1)