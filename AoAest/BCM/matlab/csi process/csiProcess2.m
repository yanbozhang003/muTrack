...
tr_idx = 28;
fileName = ['../data/spotFi_',num2str(BW),'M_',num2str(Band),'G_','tr',num2str(tr_idx)];
FILE = [fileName,'.pcap'];% capture file
...
CSI_struct = zeros(length(rx_config),1,length([seg1 seg2])-length(idle_sc),1);  % structure for real CSI
while (k <= n)
    f = p.next();
    if isempty(f)
        disp('no more frames');
...
%             if isempty(find(m_diff>20, 1))
    ...
    %             end
                CSI_mat_tmp(:,idle_sc) = [];
                CSI_struct(:,1,:,pkt_cnt) = CSI_mat_tmp;
%             else
